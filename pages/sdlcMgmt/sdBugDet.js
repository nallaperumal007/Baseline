...
import Component from "../../components/sdlcMgmt/sdBugDet";
import SdlcBugHelper from "../../helper/sdlcMgmt/sdBugDet";
// import alias_name from "../../helper/helper_source_name"
import TimelineComponent from "../../components/sdlcMgmt/sdBugDetAudit";
...
//	document.addEventListener('contextmenu', (e) => {e.preventDefault();});
  ...
  getUSDet() {
    ...
    SdlcBugHelper.getForDropDown(filter)
        .then((data) => {
            this.setState({ array_sdlcFeature: data });
        })
        .catch((err) => {
            console.log(err);
        });
  ...
  async getData() {
    const { arr_com_claim, username, tnnt_id, id_selectedStatus,id_selectedFeature } = this.state;

    try {
      if (id_selectedStatus == undefined && id_selectedFeature == undefined) {
        Swal.fire({
          icon: "warning",
  ...
  getPageDet(filter) {
    ...
    SdlcBugHelper.getPageDet(filter)
      .then((data) => {
  ...
  getFilteredData(filter) {
    SdlcBugHelper.getFilteredData(filter)
      .then((data) => {
        this.setState({
          arr_exportData: data,
          columnHeaders: {
                ...
                <div>
                    <div className={styles.rowAlignment}>
                        ...
                        <div className={styles.inputAlignment}>
                                <p>Feature Name:</p>
                                <select
                                    value={id_selectedFeature === undefined ? 0 : id_selectedFeature}
                                    onChange={(e) =>
                                        this.setState({
                                            id_selectedFeature: e.target.value,
                                        })
                                    }
                                >
                                    <option value={0} disabled selected>
                                    ...
                                    {array_sdlcFeature.map((p) => (
                                        <option value={p.feat_id}>
                                            {p.feat_name}
                                        </option>
                                    ))}
                                </select>
                  ...
                  c.clm_export == 13 && (
                    <div className={styles.excelcontainer}>
                      <ExportExcel
                        ...
                        fileName={"sdBugDet_" + moment().format("DDMMYYhhmmss")}
