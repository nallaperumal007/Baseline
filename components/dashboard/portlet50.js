...
export default class Portlet3 extends React.Component {
  ...
  getStatus() {
    ...
    PfDashboardHelper.getLibOrderStatsUC1(filter)
      .then((data) => {
        // Calculate total count from the 'count' field
        const totalCount = data.reduce((sum, row) => {
          const count = parseInt(row.count, 10); // Use 'count' instead of 'tot'
  ...
  render() {
    const { portlet34, totalCount } = this.state;
          ...
          <table className={styles.table}>
            <thead>
              <tr className={styles.table}>
                <th colSpan="2">Order Status Stats</th>
              </tr>
              <tr>
                <th>Status</th>
                <th>Count</th>
            ...
            <tbody>
              {portlet34.map((t, index) => (
                <tr key={index}>
                  ...
                  <td>
                    <a href={` ../../pfmMgmt/pfOrdM1Det?ord_status=${t.ord_status}`} target="__blank">
                      {t.count}
                    </a>
                  </td>
                </tr>
