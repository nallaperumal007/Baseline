...
import Audit_helper from "../../helper/tnntAdmin/taDmUc1";
...
export default class taDmUc1Audit extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      is_loading: false,
      ...
      <div className={styles.mainWrapper}>
        ...
        <div className={styles.wrapper} onClick={(e) => e.stopPropagation()}>
          ...
          <div className={styles.contentWrapper}>
            ...
            if (data.student_id_old !== data.student_id_new){
                 l_display_summary += "Student ID" + " is changed from " + data.student_id_old+ " to: "+ data.student_id_new + " "   ;
               }
            if (data.student_name_old !== data.student_name_new){
                 l_display_summary += "Student Name" + " is changed from " + data.student_name_old+ " to: "+ data.student_name_new + " "   ;
               }
            if (data.user_migr_opt_id_old !== data.user_migr_opt_id_new){
                 l_display_summary += "User Migration Choose" + " is changed from " + data.user_migr_opt_id_old+ " to: "+ data.user_migr_opt_id_new + " "   ;
               }
            if (data.err_code_old !== data.err_code_new){
                 l_display_summary += "Error Code" + " is changed from " + data.err_code_old+ " to: "+ data.err_code_new + " "   ;
               }
            if (data.err_msg_old !== data.err_msg_new){
                 l_display_summary += "Error Msg" + " is changed from " + data.err_msg_old+ " to: "+ data.err_msg_new + " "   ;
               }
            if (data.rec_id_old !== data.rec_id_new){
                 l_display_summary += "Parent Record ID" + " is changed from " + data.rec_id_old+ " to: "+ data.rec_id_new + " "   ;
               }
