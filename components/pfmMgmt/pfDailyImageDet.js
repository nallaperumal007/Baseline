...
export default class PfDailyImageDet extends React.Component {
  ...
  async insertRecord() {
    ...
    try {
      ...
      //Validation
      if (!validate.validateMandDate(date_display_date)) {
        alertText += ".Display Date\n"
      }
      if (!validate.validateMandText(text_image_url)) {
        alertText += ".Image Url\n"
      }
      if (!validate.validateMandTextOnly(text_image_url)) {
        alertText += ".Invalid Image Url\n"
      }
