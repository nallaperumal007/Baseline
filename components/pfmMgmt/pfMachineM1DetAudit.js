...
import Audit_helper from "../../helper/pfmMgmt/pfMachineM1Det";
...
export default class pfMachineM1DetAudit extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      is_loading: false,
      ...
      <div className={styles.mainWrapper}>
        ...
        <div className={styles.wrapper} onClick={(e) => e.stopPropagation()}>
          ...
          <div className={styles.contentWrapper}>
            ...
            if (data.machine_nr_old !== data.machine_nr_new){
                 l_display_summary += "Machine Nr" + " is changed from " + data.machine_nr_old+ " to: "+ data.machine_nr_new + " "   ;
               }
            if (data.model_type_id_old !== data.model_type_id_new){
                 l_display_summary += "Model Type" + " is changed from " + data.model_type_id_old+ " to: "+ data.model_type_id_new + " "   ;
               }
            if (data.clt_id_old !== data.clt_id_new){
                 l_display_summary += "Client Name" + " is changed from " + data.clt_id_old+ " to: "+ data.clt_id_new + " "   ;
               }
            if (data.installation_date_old !== data.installation_date_new){
                 l_display_summary += "Installation Date" + " is changed from " + data.installation_date_old+ " to: "+ data.installation_date_new + " "   ;
               }
            if (data.installed_by_old !== data.installed_by_new){
                 l_display_summary += "Installed By" + " is changed from " + data.installed_by_old+ " to: "+ data.installed_by_new + " "   ;
               }
            if (data.warranty_end_date_old !== data.warranty_end_date_new){
                 l_display_summary += "Warranty End Date" + " is changed from " + data.warranty_end_date_old+ " to: "+ data.warranty_end_date_new + " "   ;
               }
